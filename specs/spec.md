...
- Per-seller trend-card ownership + a "My Trend Cards" tab (Phase 2). Today (P10, FR9) seller-authored cards persist to a GLOBAL, shared `trend_cards` warehouse: the table has no `seller_id` column and the `TrendCard` contract has no `sellerId` field, so a card authored by one seller becomes a shared cache hit for the next seller (by design, FR9). There is NO personalized per-seller collection and NO "My Trend Cards" tab beside Discover. Deferred work: (a) a schema change to associate cards with a seller (add `seller_id` to `trend_cards` or a `seller_trend_cards` join table) — requires an explicit contracts/migrations unlock, since `packages/contracts/*` is frozen; (b) inject `sellerId` from the authenticated session on the write path (never trust the client) — shares the seller-auth dependency with the deferred credit-metering item above; (c) a per-seller read/list path + a new UI tab/route (`My Trend Cards`) beside Discover. Trade-off to decide at planning time: keep the shared warehouse (optimizes cross-seller cache reuse) vs. per-seller ownership (each seller sees only their own cards, losing shared-cache benefit) — likely both, i.e. a shared warehouse plus a per-seller "authored by me" view. Note: `seller_projects` (generated designs) IS already per-seller; only the trend cards themselves are not.
